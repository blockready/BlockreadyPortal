...
// api/create-landing-conversion.ts
...
var onRequestPost5 = /* @__PURE__ */ __name(async (context) => {
  ...
  `).eq(
    ...
    const {
      data: file,
      error: fileError
    } = await supabase.from("resource_files").select(`
    file_name,
    mime_type,
    r2_key
  `).eq(
    ...
    const object = await context.env.RESOURCES_BUCKET.get(
      file.r2_key
    );
    if (!object) {
      return new Response(
    ...
    return Response.json(
      {
        success: false,
        error: error instanceof Error ? error.message : JSON.stringify(
          error
        )
      },
      {
...
// api/track-resource-activity.ts
var onRequestPost8 = /* @__PURE__ */ __name(async (context) => {
  try {
    ...
    const { error } = await supabase.from(
      ...
      user_id: userId,
      resource_id: resource.id,
      activity_type: activityType ?? "resource_opened",
      resource_slug: resource.slug,
      resource_title: resource.title,
      category: resource.category,
      resource_type: resource.resource_type
...
// api/update-landing-conversion.ts
...
var onRequestPost10 = /* @__PURE__ */ __name(async (context) => {
  try {
    ...
    const {
      data: profile,
      error: fetchError
    } = await supabase.from("profiles").select("*").eq(
      "user_id",
      userId
    ).single();
    if (fetchError) {
    ...
    const {
      error: updateError
    } = await supabase.from("profiles").update(updates).eq(
      "user_id",
      userId
    );
    if (updateError) {
...
var onRequestPost11 = /* @__PURE__ */ __name(async (context) => {
  try {
    ...
    const {
      data: lead,
      error: leadError
    } = await supabase.from("leads").select("email_verified").eq("user_id", userId).single();
    if (leadError) {
      throw leadError;
    }
    ...
    if (landingConversionId && userId) {
      const { error } = await supabase.from("resource_landing_conversions").update({
        user_id: userId,
        action: "signup",
...
// ../.wrangler/tmp/pages-XVssyN/functionsRoutes-0.6918689202766297.mjs
var routes = [
  {
  ...
  {
    routePath: "/api/create-landing-conversion",
    mountPath: "/api",
    method: "POST",
    middlewares: [],
  ...
  {
    routePath: "/api/update-landing-conversion",
    mountPath: "/api",
    method: "POST",
    middlewares: [],
...
// ../.wrangler/tmp/bundle-2BQQAJ/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-2BQQAJ/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.36455295261833354.mjs.map
